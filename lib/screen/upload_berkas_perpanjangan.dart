...
class _UploadBerkasPerpanjanganState extends State<UploadBerkasPerpanjangan> {
  ...
  bacaDataAlasanReklame() {
    fetchDataAlasan().then((value) {
      Map json = jsonDecode(value);
      alasan = json['data'][0]['alasan'];
      print(alasan);
      setState(() {});
    });
  }
  ...
  Future<String> fetchDataAlasan() async {
    final response = await http.post(
        Uri.parse("http://10.0.2.2:8000/api/read_alasan_reklame"),
        body: {'id_reklame': widget.reklame_id.toString()});
    if (response.statusCode == 200) {
      return response.body;
    } else {
